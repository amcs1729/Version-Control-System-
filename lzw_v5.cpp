...
///
    int do_compress(int choice, std::string filename, std::string target) {
        ...
        if (choice == 1)
            ...
            print_usage(std::string("input_file `") + filename + "' could not be opened.");
            return EXIT_FAILURE;
        }
        ...
        output_file.open(target, std::ios_base::binary);

        if (!output_file.is_open()) {
            print_usage(std::string("output_file `") + target + "' could not be opened.");
            return EXIT_FAILURE;
        }
